...
class _EmployeeDetailScreenState extends State<EmployeeDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final balanceLabel = employee.currentBalance == 0
        ? 'برابر'
        : (employee.currentBalance > 0 ? 'ملازم کو دینا ہے (بقایا تنخواہ/بونس)' : 'ملازم سے لینا ہے (ایڈوانس)');
              ...
              child: ListView(
                ...
                children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('ماہانہ تنخواہ: ${AppFormat.currency(employee.monthlySalary)}',
                  ...
                  Wrap(
                    ...
                    children: [
                      OutlinedButton.icon(
                        onPressed: () => _addTransaction(AppConstants.empTxnSalaryPayment, 'تنخواہ ادا کریں'),
                        icon: const Icon(Icons.payments_outlined, size: 18),
                        label: const Text('تنخواہ ادا کریں'),
                      ),
                      OutlinedButton.icon(
                        onPressed: () => _addTransaction(AppConstants.empTxnAdvance, 'ایڈوانس دیں'),
                        icon: const Icon(Icons.request_quote_outlined, size: 18),
                        label: const Text('ایڈوانس دیں'),
                      ),
                      OutlinedButton.icon(
                        onPressed: () => _addTransaction(AppConstants.empTxnBonus, 'بونس شامل کریں'),
                        icon: const Icon(Icons.card_giftcard_outlined, size: 18),
                        label: const Text('بونس'),
                      ),
                      OutlinedButton.icon(
                        onPressed: () => _addTransaction(AppConstants.empTxnDeduction, 'کٹوتی کریں'),
                        icon: const Icon(Icons.remove_circle_outline, size: 18),
                        label: const Text('کٹوتی'),
                      ),
                    ],
                  ),
                  const SizedBox(height: 20),
                  Text('آج کی حاضری', style: AppFonts.body(fontSize: 13.5, weight: FontWeight.w700)),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      Expanded(child: _attendanceButton('حاضر', AppConstants.attendancePresent, AppColors.success, markedToday)),
                      const SizedBox(width: 8),
                      Expanded(child: _attendanceButton('غیر حاضر', AppConstants.attendanceAbsent, AppColors.danger, markedToday)),
                      const SizedBox(width: 8),
                      Expanded(child: _attendanceButton('رخصت', AppConstants.attendanceLeave, AppColors.gold500, markedToday)),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Wrap(
                    ...
                    children: [
                      for (final a in _monthAttendance.reversed.take(14))
                        Chip(
                          label: Text('${a.dateKey.substring(8)}: ${a.label}', style: const TextStyle(fontSize: 11)),
                  ...
                  if (_transactions.isEmpty)
                    ...
                    for (final t in _transactions.reversed) _txnTile(t),
                ],
              ),
            ),
    );
  ...
  Widget _txnTile(EmployeeTransaction t) {
    final color = t.amount > 0 ? AppColors.danger : AppColors.success;
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(t.label, style: AppFonts.body(fontSize: 13, weight: FontWeight.w600)),
                Text(dateStr, style: AppFonts.body(fontSize: 11, color: AppColors.inkSoft)),
                if (t.note != null) Text(t.note!, style: AppFonts.body(fontSize: 11, color: AppColors.inkSoft)),
              ],
            ),
          ),
          Text(
            '${t.amount >= 0 ? '+' : '-'}${AppFormat.currency(t.amount.abs())}',
