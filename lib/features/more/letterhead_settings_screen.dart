...
class _LetterheadSettingsScreenState extends State<LetterheadSettingsScreen> {
  ...
  Future<void> _save() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('بل کی سیٹنگز محفوظ ہو گئیں')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('بل کی سیٹنگز')),
      body: ListView(
        ...
        children: [
          Container(
            ...
            child: Text(
              'یہ سیٹنگز محفوظ ہو جائیں گی۔ اصل PDF بل پر ان کا اطلاق فیز 4 میں شامل ہوگا۔',
              style: AppFonts.body(fontSize: 11.5, color: AppColors.teal900),
            ),
          ),
          const SizedBox(height: 18),

          Text('لوگو', style: AppFonts.body(fontSize: 13, color: AppColors.teal800, weight: FontWeight.w700)),
          const SizedBox(height: 8),
          OutlinedButton.icon(
            onPressed: null, // needs an image-picker dependency — Phase 4
            icon: const Icon(Icons.image_outlined),
            label: const Text('لوگو اپلوڈ کریں (فیز 4)'),
          ),
          const SizedBox(height: 10),
          Text('لوگو کی پوزیشن', style: AppFonts.body(fontSize: 12.5, color: AppColors.inkSoft)),
          Row(
            children: [
              Expanded(
                child: RadioListTile<String>(
                  ...
                  title: const Text('دائیں کونا', style: TextStyle(fontSize: 13)),
              ...
              Expanded(
                child: RadioListTile<String>(
                  ...
                  dense: true,
                  contentPadding: EdgeInsets.zero,
                  title: const Text('بائیں کونا', style: TextStyle(fontSize: 13)),
                  onChanged: (v) => setState(() => _corner = v!),
                ),
              ),
            ],
          ...
          Text('دکاندار کا نام', style: AppFonts.body(fontSize: 13, color: AppColors.teal800, weight: FontWeight.w700)),
          const SizedBox(height: 8),
          TextField(controller: _nameCtrl, onChanged: (_) => setState(() {}), decoration: const InputDecoration(hintText: 'مثلاً: الرحمن جنرل سٹور')),
          const SizedBox(height: 10),
          Text('فونٹ', style: AppFonts.body(fontSize: 12.5, color: AppColors.inkSoft)),
          Wrap(
            spacing: 8,
          ...
          const SizedBox(height: 16),
          Text('درمیانی واٹرمارک', style: AppFonts.body(fontSize: 13, color: AppColors.teal800, weight: FontWeight.w700)),
          RadioListTile<String>(
            value: 'none',
            groupValue: _watermarkType,
            dense: true,
            contentPadding: EdgeInsets.zero,
            title: const Text('کوئی نہیں', style: TextStyle(fontSize: 13)),
          ...
          RadioListTile<String>(
            ...
            dense: true,
            contentPadding: EdgeInsets.zero,
            title: const Text('پورا نام / لوگو', style: TextStyle(fontSize: 13)),
            onChanged: (v) => setState(() => _watermarkType = v!),
          ),
          RadioListTile<String>(
            value: 'initials',
            ...
            dense: true,
            contentPadding: EdgeInsets.zero,
            title: const Text('مخفف (initials)', style: TextStyle(fontSize: 13)),
            onChanged: (v) => setState(() => _watermarkType = v!),
          ),
          if (_watermarkType != 'none') ...[
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: _watermarkType == 'initials' ? 'مثلاً: TK' : 'پورا نام',
              ),
            ),
          ],
          const SizedBox(height: 8),
          TextField(
            ...
            onChanged: (_) => setState(() {}),
            decoration: const InputDecoration(hintText: 'موبائل نمبر (واٹرمارک کے ساتھ دکھایا جائے گا)'),
          ),

          const SizedBox(height: 20),
          Text('Live Preview', style: AppFonts.body(fontSize: 13, color: AppColors.teal800, weight: FontWeight.w700)),
          const SizedBox(height: 8),
          _preview(),

          const SizedBox(height: 20),
          ElevatedButton(onPressed: _save, child: const Text('محفوظ کریں')),
        ],
      ),
    );
  }
      ...
      child: Stack(
        children: [
          ...
          Align(
            ...
            child: Text(
              _nameCtrl.text.isEmpty ? 'دکان کا نام' : _nameCtrl.text,
